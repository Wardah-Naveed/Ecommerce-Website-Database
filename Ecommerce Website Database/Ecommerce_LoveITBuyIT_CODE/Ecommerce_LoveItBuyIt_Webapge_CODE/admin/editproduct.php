  ...
  if (!isset($_GET['product_id'])) {
    ...
    if(isset($_POST['title']) && isset($_POST['category']) && isset($_POST['price']) && isset($_POST['inventory']) && isset($_POST['content'])) 
    {
        ...
        if(!empty($_FILES['image3']['tmp_name'])) {
            // get image data
            $image1Data = addslashes(file_get_contents($_FILES['image3']['tmp_name']));
            $sql = "UPDATE product SET image3 = '$image1Data' WHERE product_id = '$product_id'";
            $result = $conn->query($sql);
        } 
        $title = $_POST['title'];
        ...
        $sql = "UPDATE product SET product_name = '$title', category_id = '$catId', price = '$price', inventory = '$inventory', product_description = '$content', discount = '$discount' WHERE product_id = '$product_id'";
        $result = $conn->query($sql);
        if($result)
        {
    ...
    <div class="content">
      ...
      <form action="" method="POST" enctype="multipart/form-data">
        <div class="row">
          ...
          <div class="col-75">
            <input type="text" id="title" name="title" value="<?php echo $title ?>" placeholder="Enter Product name.." required>
          </div>
        </div>
        <div class="row">
          <div class="col-25">
            ...
            <select id="category" name="category">
              <?php
                ...
                while ($row = mysqli_fetch_array($result)) { ?>
                  <option value="<?php echo $row['category_name'] ?>" <?php if ($category == $row['category_id']) echo "selected"; ?>><?php echo $row['category_name'] ?></option>
              <?php } ?>
            </select>
          </div>
        </div>
        <div class="row">
            ...
            <div class="col-75">
              <input type="number" id="price" name="price" value="<?php echo $price ?>" min="1" required>
            </div>
          </div>
        <div class="row">
          <div class="col-25">
          ...
          <div class="col-75">
            <input type="number" id="inventory" name="inventory" value="<?php echo $inventory ?>" min="0" required>
          </div>
        </div>
        <div class="row">
          <div class="col-25">
            <label for="Inventory">Discount %age</label>
          </div>
          <div class="col-75">
            <input type="number" id="discount" name="discount" value="<?php echo $discount ?>" min="0" required>
          </div>
        </div>
        <div class="row">
          <div class="col-25">
          ...
          <div class="col-75">
            <section>
              <input id="upload" type="file" name="image1"/>
            </section>
          </div>
        </div>
        <div class="row">
          ...
          <div class="col-75">
            <section>
              <input id="upload" type="file" name="image2"/>
            </section>
          </div>
        </div>
        <div class="row">
          ...
          <div class="col-75">
            <section>
              <input id="upload" type="file" name="image3"/>
            </section>
          </div>
        </div>
        <div class="row">
